...
<tr>
<td class="cart-product-remove delete_cart_value" data-reproductid="' . htmlspecialchars($items['rowid']) . '">x</td>
    <td>
    ...
    <td>₹' . number_format($items['price'], 2) . '</td>
      <td class="product-quantity"><div class="input-group">
                      <input class="quantity form-control" type="number" min="1" max="100000"  value="' . $items['qty'] . '" data-reproductid="' . htmlspecialchars($items['rowid']) . '"  >
...
function build_cart_totals($subTotal, $shipping, $discount, $totalWithGST, $exclusiveGST)
{
    return '
        ...
        <tr>
            <td>Exclusive GST</td>
            <td>₹' . number_format($exclusiveGST, 2) . '</td>
        </tr>' : '') . '
        <tr>
            <td>Shipping</td>
        ...
        ' . ($discount > 0 ? '
        <tr>
            <td>Discount <a href="#" class="remove_discount" title="Remove Coupon">Remove Coupon</a></td>
    ...
    if (count($availableCart) > 0) {
        foreach ($availableCart as $items) {
            ...
            $output .= '  <div class="single-cart">
                      <div class="cart-img"> <a href="#"><img src="' . $productImage . '" alt="book" /></a> </div>
                      <div class="cart-info">
                        <h5><a href="#">' . htmlspecialchars($items['name']) . '</a></h5>
                        <p>' . htmlspecialchars($items['qty']) . ' x ₹' . number_format($items['price'], 2) . '</p>
                      </div>
                      <div class="cart-icon"> <a href="#" class="delete_cart_value" data-reproductid="' . htmlspecialchars($items['rowid']) . '" id="remove' . htmlspecialchars($items['rowid']) . '" ><i class="fa fa-remove"></i></a> </div>
                    </div>
...
// Hamesha array banane ke liye (int, string, array sab handle karega)
if (!is_array($selected)) {
  ...
  $categories = $commanmodel->all_multiple_query_order_by('category', [], 'category_id', 'ASC');
    $categoryTree = buildCategoryTree($categories, $parentId);
...
if (!empty($selected)) {
    foreach ($selected as $selectedId) {
        $expandedCategories[] = $selectedId;

        // Try to get the current category safely
        $currentCategory = $commanmodel->get_single_query('category', ['category_id' => $selectedId]);
        if ($currentCategory) {
            // Ensure all parents are included in expanded categories
            while (!empty($currentCategory->parent_id) && $currentCategory->parent_id > 0) {
                $expandedCategories[] = $currentCategory->parent_id;
                $currentCategory = $commanmodel->get_single_query('category', ['category_id' => $currentCategory->parent_id]);

                // Agar parent record nahi mila to break
                if (!$currentCategory) {
                    break;
                }
            }
...
// Duplicate IDs hatao
$expandedCategories = array_unique($expandedCategories);
        ...
        <div class="tree_main">
            <ul id="bs_main">
                ...
                function renderCategoryTree($tree, $selected, $expandedCategories) {
                    $output = '';
                    foreach ($tree as $category) {
                        $isChecked = in_array($category->category_id, $selected) ? 'checked' : '';
                        $isExpanded = in_array($category->category_id, $expandedCategories) ? 'open' : '';
                        $isSelected = in_array($category->category_id, $selected) ? 'selected-category' : '';
                        $isExpandeds = in_array($category->category_id, $expandedCategories) ? 'display: block;' : 'display: none;';
