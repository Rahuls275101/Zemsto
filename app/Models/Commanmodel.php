...
class Commanmodel extends Model
{
    ...
    public function __construct()
    {
     ...
     public function getExoticData($table, $filters = [], $order = null, $limit = null, $offset = 0)
    {
        ...
        // Apply ordering
        if ($order !== null) {
            $builder->orderBy($order['column'], $order['order']);
        }
        ...
        // Get gallery images for each collection
        foreach ($results as $result) {
            $galleryBuilder = $this->db->table('exotic_gallery');
            $galleryBuilder->where('exotic_id', $result->exotic_id);
            $galleryBuilder->orderBy('display_order', 'ASC');
            $gallery = $galleryBuilder->get()->getResult();
    ...
    /**
     * Alternative method using query builder directly
     */
    public function getExoticCollections($filters = [], $order = null, $limit = null, $offset = 0)
    {
        $builder = $this->db->table('exotic_collection');
    ...
    /**
     * Get single exotic collection by slug
     */
    public function getExoticBySlug($slug)
    {
        $builder = $this->db->table('exotic_collection');
        $builder->where('url_slug', $slug);
        $builder->where('exotic_status', 'Active');
        ...
        if ($result) {
            ...
            $galleryBuilder->where('exotic_id', $result->exotic_id);
            $galleryBuilder->orderBy('display_order', 'ASC');
            $result->gallery_images = $galleryBuilder->get()->getResult();
        }
...
public function product_rating($course_id)
{
    ...
    if ($review_count > 0) {
        ...
        foreach ($query->getResult() as $row) {
            ...
            for ($i = 1; $i <= 5; $i++) {
                if ($i <= $rating) {
                    $stars_html .= '<li><a href="#"><i class="fa fa-star"></i></a></li>';
                } elseif ($i - 0.5 == $rating) {
                    $stars_html .= '<li><a href="#"><i class="fa fa-star-half-alt"></i></a></li>';
                } else {
                    $stars_html .= '<li><a href="#"><i class="fa fa-star"></i></a></li>';
                }
            }
            ...
            $reviews_output .= '
                <li>
                   <div class="review-right" style="padding-left:0px">
                                                    <div class="review-content">
                                                        <h4>' . $user_name . ' </h4>
                                                    </div>
                                                    <div class="review-details">
                                                        <p class="review-author">Review by<a href="#">' . $message . '</a></p>
                                                        <p class="review-date">Posted on <span>' . $review_date . '</span></p>
                                                    </div>
                                                </div>
                   ';
        }
        ...
        for ($i = 1; $i <= 5; $i++) {
            if ($i <= $rating_round) { 
                ...
                $output .= '<a href="#"><i class="fa fa-star-half-alt"></i></a>';
            } else {
                $output .= '<a href="#"><i class="fa fa-star"></i></a>';
            }
        }
